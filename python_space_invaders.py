...
def game_over():
    ...
    if check_if_current_score_is_top10(high_scores):
        add_current_user_to_database(username=inpt_name,
                                     user_score = GameState.instance().score)
        high_scores = get_high_scores()
...
def get_high_scores() -> dict:
    get_request = requests.get('http://127.0.0.1:5000/get_records')
    dictionary_high_records = get_request.json()
    return dictionary_high_records

def check_if_current_score_is_top10(high_scores_dict: dict):
    for player_name, player_score in high_scores_dict.items():
        if GameState.instance().score > player_score or len(high_scores_dict) < 10:
            return True

def add_current_user_to_database(username: str, user_score: int):
    user_json_info = json.dumps({username: user_score})
    requests.post('http://127.0.0.1:5000/', data=user_json_info)

def print_high_scores(high_scores_dict: dict):
    print('Top 10 scores:')
    for player_name, player_score in high_scores_dict.items():
